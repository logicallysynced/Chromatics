...
{
    public class BPMCircularPulseEffect : AbstractUpdateAwareDecorator, ILedGroupDecorator
    {
        ...
        public BPMCircularPulseEffect(ListLedGroup _ledGroup, int bpm, double beatsPerCycle, double pulseRadius, double fadeWidth, Color[] colors, RGBSurface surface, Color baseColor = default) : base(surface, updateIfDisabled: false)
        {
        ...
        protected override void Update(double deltaTime)
        {
            try
            {
                ...
                // Derive speed: pulse reaches max radius in (beatsPerCycle * _beatInterval) seconds
                double speedPerSec = pulseRadius / (beatsPerCycle * _beatInterval);
                for (int i = _pulses.Count - 1; i >= 0; i--)
                {
                    _pulses[i].CurrentRadius += speedPerSec * deltaTime;
            ...
            {
                Debug.WriteLine($"BPMCircularPulseEffect: {ex.Message}");
            }
        }
