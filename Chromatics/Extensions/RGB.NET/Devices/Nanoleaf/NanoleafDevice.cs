...
namespace Chromatics.Extensions.RGB.NET.Devices.Nanoleaf
{
    public class NanoleafDevice : AbstractRGBDevice<NanoleafDeviceInfo>
    {
        ...
        public NanoleafDevice(NanoleafDeviceInfo info, NanoleafUpdateQueue updateQueue, NanoleafClientDefinition def, IReadOnlyList<NanoleafPanelPosition> panels)
        ...
        // One LED per panel at its real layout centroid. Panels are ordered
        // by ascending panelId (the queue's panelOrder uses the same sort),
        // so LedId.Custom1..N line up with the stream frame indices and
        // survive restarts. Nanoleaf coordinates are y-up; we flip to y-down
        // to match RGB.NET / screen space, and normalise so the minimum
        ...
        private void InitializeLayout()
        {
            ...
            foreach (var p in _panels)
            {
                ...
                var led = AddLed((LedId)((int)LedId.Custom1 + i), new Point(x, y), new Size(60));
